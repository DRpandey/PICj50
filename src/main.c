...
*/
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                ...
                {
                    for (i=0; i<4; i++){
                        I2C_Frame.data[i] = msgbuffer[i];
                    }

                    I2C_Frame.device = 1;
                    I2C_Frame.parity = msgbuffer[0] ^ msgbuffer[1] ^ msgbuffer[2] ^ msgbuffer[3];
                    I2C_Frame.count = msg_count++;
                    FromMainLow_sendmsg(8,MSG_ADC_DATA,&I2C_Frame);
                };
                case MSGT_OVERRUN:
                case MSGT_UART_DATA:
                {
